...
# ------------------------------------Recombination Steps---------------------------------------------------------
    ...
    # Thực hiện một quá trình điều chỉnh (origin shift) để xử lý các giá trị độ thích nghi (fitness) âm trong quần thể nhiễm sắc thể.
...
# ----------------------------Start Training---------------------------------------------------------------
        ...
        for generation in range(0, noOfGeneration):
            ...
            # Calculate Fitnesses
            for chromosome in chromosomes:
                fitnessValue = fitnessFunction(chromosome)   # tính toán độ thích nghi
                chromosome.originalFitness = fitnessValue

                # Nếu giải thuật hướng tới việc tìm kiếm giải pháp với độ thích nghi càng thấp càng tốt (minimize), đảo ngược giá trị độ thích nghi. Thực hiện điều chỉnh nếu có giá trị độ thích nghi âm.
            ...
            # Create next Generation by selection: Tạo Quần Thể Thế Hệ Tiếp Theo: 
            # Tạo quần thể mới bằng cách thêm các nhiễm sắc thể đã qua lai ghép và đột biến, đồng thời đảm bảo tính độc nhất của gen. 
...
#------------------------------Add Elites to next Generation--------------------------------------
            ...
            # In ra giá trị fitness của cá thể tốt nhất tại thế hệ hiện tại
            # print(f"Generation {self.GENERATION_COUNT}, Best Fitness: {bestIndividual.fitness}")
